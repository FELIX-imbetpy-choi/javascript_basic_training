...
// "숫자형"
// 모던 자바스크립트는 숫자를 나타내는 두가지 자료형을 지원합니다.나타내는
// 1. 일반적인 숫자는 '배정밀도 부동소수점 숫자(double precision floating point number)'로 알려진 64비트 형식의 IEEE-754에 저장됩니다. 
// 튜토리얼 전체에서 이 형식을 사용하여 숫자를 표현할 예정입니다.
// 2. 임의의 길이를 가진 정수는 BigInt 숫자로 나타낼 수 있습니다.
// 일반적인 숫자는 253이상이거나 -253이하일 수 없다는 제약 때문에 BigInt라는 새로운 자료형이 만들어졌습니다. BigInt는 아주 특별한 경우에만 사용되므로, 
// 별도의 챕터 BigInt에서 자세한 내용을 다루겠습니다.
...
// 그런데 이렇게 0을 많이 사용해 숫자를 표현하다 보면 잘못 입력하기 쉽기 때문에,
//  실제로는 이런 방법을 잘 사용하지 않습니다. 
//  0을 많이 입력하는 게 귀찮기도 하지요. 
//  그래서 대개는 10억(billion)을 나타낼 땐 '1bn'을 사용하고, 
//  73억을 나타낼 땐 '7.3bn'을 사용합니다.
//  큰 숫자를 나타낼 땐 이런 방법이 주로 사용되죠.
// 자바스크립트에서도 숫자 옆에 'e'를 붙이고 0의 개수를 그 옆에 붙여주면 숫자를 줄일 수 있습니다.
billion = 1e9;  // 10억, 1과 9개의 0
...
console.log( a == b ); // true, 진법은 다르지만, a와 b는 같은 수임
...
//소수점 두 번째 자리 숫자까지만 남기고 싶은 경우, 
// 숫자에 100 또는 100보다 큰 10의 거듭제곱 수를 곱한 후, 
...
num = 12.34;
console.log( num.toFixed(5) ); // "12.34000", 소수부의 길이를 5로 만들기 위해 0이 추가되었습니다.
...
// 인터넷 쇼핑몰을 구축 중이고 $0.30를 보여줘야 할 때 유용하죠. 
// 문자형으로 바뀐 숫자를 다시 숫자형으로 강제 변환하려면 단항 덧셈 연산자를 사용하면 됩니다.
 sum = 0.1 + 0.2;
console.log( +sum.toFixed(2) ); // 0.3

//구현을 하다 보면 무한 소수가 나오는 경우를 완전히 차단해야 하는 경우가 생기곤 합니다. 
// 달러가 아닌 센트 단위로 물품 가격을 저장하는 쇼핑몰을 담당하고 있는데, 
// 행사 때문에 가격을 30% 할인해야 하는 경우가 그렇죠. 무한소수를 방지하는 완벽한 방법은 사실 없습니다. 
// 필요할 때마다 '꼬리’를 잘라 어림수를 만드는 방법뿐이죠.

// 흥미로운 발견
// 숫자가 스스로 증가하네요!
// 문제의 원인은 역시나 정밀도 손실 때문입니다. 숫자를 저장할 땐 64비트가 사용되는데, 
// 이 중 실제 숫자를 저장하는 데 사용되는 52비트에 위 숫자를 저장하기엔 공간이 모자랍니다.
...
// 안타깝게도 대답은 '필요하다’입니다. NaN은 NaN 자기 자신을 포함하여 그 어떤 값과도 같지 않다는 점에서 독특합니다.
...
console.log( isFinite("str") ); // false, NaN이기 때문입니다.
console.log( isFinite(Infinity) ); // false, Infinity이기 때문입니다.
...
// 내부 알고리즘에서 두 값을 비교해야 하는데, 비교 결과가 정확해야 하는 경우 Object.is를 사용하죠. 
// Object.is에서 사용되는 비교방식은 명세서에서 SameValue라고 불립니다.
...
console.log( parseInt('a123') ); // NaN, a는 숫자가 아니므로 숫자를 읽는 게 중지됩니다.

// parseInt(str, radix)의 두 번째 인수
// parseInt()의 두 번째 매개 변수는 선택적으로 사용할 수 있습니다. 
// radix는 원하는 진수를 지정해 줄 때 사용합니다. 따라서 parseInt를 사용하면 16진수 문자열, 2진수 문자열 등을 파싱할 수 있습니다.
